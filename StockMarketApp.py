...
with st.echo():
        ...
        model = Sequential()
        # Input layer
        model.add(Bidirectional(LSTM(n_neurons, activation='relu', return_sequences=True),
...
def plot_future_data():
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=original['Date'], y=original['Open'], name="Historical Trend"))
        fig.add_trace(go.Scatter(x=df_forecast['Date'], y=df_forecast['Open'], name="Forecast"))
